...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    if ($newpassword != $re_new_password) {
        ...
        $ret = mysqli_query($con, "SELECT ADMIN_ID FROM admin_data WHERE Email='$emailid' AND USERNAME='$username'");
        $num = mysqli_num_rows($ret);
        if ($num > 0) {
            $query = mysqli_query($con, "UPDATE admin_data SET PASSWORD='$newpassword' WHERE Email='$emailid' AND USERNAME='$username'");
            if ($query) {
